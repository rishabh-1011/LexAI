...
def retrieve_from_document(question: str, document_text: str, top_k: int = 4) -> str:
    ...
    doc_chunks = _chunk_text(document_text, chunk_size=800, overlap=150)

    if not doc_chunks:
        return ""

    relevant = _tfidf_retrieve(question, doc_chunks, top_k, threshold=0.02)
    return "\n\n---\n\n".join(relevant)

def retrieve_for_qa(question: str, document_text: str) -> tuple:
    """
    Dual retrieval for Q&A — searches both document and KB.
    Returns (doc_context, kb_context) tuple.
    """
    doc_context = retrieve_from_document(question, document_text)
    kb_context  = retrieve_relevant_knowledge(question)
